...
struct PrintVisitor : boost::static_visitor<std::string>
{
  ...
  std::string apply(const boost::variant<Ts...>& value, bool inc_indent = true) const
  {
    return boost::apply_visitor(PrintVisitor(indent + (inc_indent ? "  " : "")), value);
  }
  ...
  template <typename T>
  std::string apply(const boost::optional<T>& value, bool inc_indent = true) const
  {
